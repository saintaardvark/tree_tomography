...
$(ELF): $(SRC) $(HDRS)
	...
	echo $$SHELL
	which arduino-cli
	~/bin/arduino-cli compile -b $(FQBN) $(VFLAG)
...
upload:
	...
	~/bin/arduino-cli upload -b $(FQBN) -p $(SERIAL_DEV) $(VFLAG); \
	fi

ota:
	@PLAT_PATH=`~/bin/arduino-cli compile -b $(FQBN) --show-properties | grep '^runtime.platform.path' | awk -F= '{print $$2}'` ; \
	   PY_PATH=`~/bin/arduino-cli compile -b $(FQBN) --show-properties | grep '^runtime.tools.python3.path' | awk -F= '{print $$2}'` ; \
